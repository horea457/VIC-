...
apply_css()
st.caption('V4.1 · 실제 사후검증 DB 연결됨 · 17건')
page_header(
    'PATTERN LAB',
    '성공·실패 패턴',
...
with verified_tab:
    ...
    if selected:
        ...
        if data:
            ...
            if srows:
                selected_idea_id=data[srows[0]]['idea_id']
                st.session_state['v41_selected_verified_idea']=selected_idea_id
                idea_quick_view(selected_idea_id)
            fallback_id=st.session_state.get('v41_selected_verified_idea')
            if fallback_id:
