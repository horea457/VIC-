...
with gzip.open(tmp,'rt',encoding='utf-8-sig',errors='replace',newline='') as f:
    for row in csv.DictReader(f):
        ...
        if any(k in blob for k in ['BJRI','BJ\'S RESTAURANTS','BJS RESTAURANTS','PETS','PETMED EXPRESS','PETMED']):
            rows.append(row)
out=ROOT/'.vic_tmp'/'b032_source.json'; out.parent.mkdir(parents=True,exist_ok=True)
